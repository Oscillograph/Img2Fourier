...
{	
	...
	void Image::Load(const std::string& filepath)
	{
		...
		// unsigned char* pixel = pixels; // unused for some reason
		uint32_t p, r, g, b, a;
		p = 4294967295; // 256 * 256 * 256 * 256 - 1;
		r = 256 * 256 * 256;
		g = 256 * 256;
		b = 256;
		a = 1;
...
//		SAVANNAH_CONSOLE_LOG(colors.size());
	...
	Img2FFT::Img2FFT()
	{
		...
		m_fileDialogInfo.fileExtensions = {
			const_cast<char*>(std::string(".PNG").c_str()), 
			const_cast<char*>(std::string(".jpg").c_str()), 
			const_cast<char*>(std::string(".bmp").c_str()), 
			const_cast<char*>(std::string(".gif").c_str()),
		};
		...
		for (auto it = m_ColorSchemesMap.begin(); it != m_ColorSchemesMap.end(); ++it)
		{
			...
			for (size_t i = 0; i < m_ColorSchemesMap[it->first].size(); ++i)
			{
	...
	void Img2FFT::SetupResources()
	{
		...
		// fftw stuff
		FFTPlan();
		SAVANNAH_CONSOLE_LOG("FFTW plan created");
...
//							int offset = 0;
						...
						SAVANNAH_CONSOLE_LOG("FFTW max: ", m_Image->fftw_data_max, "; FFTW min: ", m_Image->fftw_data_min);
...
//					SAVANNAH_CONSOLE_LOG("Total colors: ", m_Image->colors.size());
//					
...
//						});
//					SAVANNAH_CONSOLE_LOG("Sorted colors");
//					
...
//					SAVANNAH_CONSOLE_LOG("Colors in Native colormap: ", m_ColorSchemes["Native"].colors.size());
//					for (int k = 0; k < m_ColorSchemes["Native"].colors.size(); ++k)
//					{
//						SAVANNAH_CONSOLE_LOG("Native color #", k, 
